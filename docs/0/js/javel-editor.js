...
class JavelEditor {
    constructor() {
        ...
        //this.htmls = van.derive(()=>{this.parser.toHtml(this.textBlocks.val);this.viewer.children = div(this.htmls);})
        this.size = van.derive(()=>this.parser.calcSize(this.htmls.val))
    }
    init() {
        this.viewer = new SingleScreen() 
        //van.derive(()=>this.viewer.children = this.htmls)
        //van.derive(()=>this.viewer.children = div(this.htmls))
        //van.derive(()=>this.viewer.children = [()=>div(this.htmls)])
        //this.viewer.children = [()=>div(this.htmls)]
        //this.viewer.children = ()=>div(this.htmls)
        //this.viewer.children = div(this.htmls)
        this.viewer.children = this.htmls
        console.log(this.viewer.children)
        console.log(this.viewer.el)
        //van.add(document.body, div({style:()=>this.style()}, textarea({rows:5, cols:40, oninput:(e)=>this.manuscript.val=e.target.value}, ()=>this.manuscript.val), button({style:`word-break:break-all;padding:0;margin:0;line-height:1em;letter-spacing:0;`},()=>`${this.size.val}字`), ()=>div([div('AAAA'), div('BBBB')])))
        //van.add(document.body, div({style:()=>this.style()}, textarea({rows:5, cols:40, oninput:(e)=>this.manuscript.val=e.target.value}, ()=>this.manuscript.val), button({style:`word-break:break-all;padding:0;margin:0;line-height:1em;letter-spacing:0;`},()=>`${this.size.val}字`), ()=>div(this.htmls)))
        ...
        //van.add(document.body, div({style:()=>this.style()}, textarea({rows:5, cols:40, oninput:(e)=>this.manuscript.val=e.target.value}, ()=>this.manuscript.val), button({style:`word-break:break-all;padding:0;margin:0;line-height:1em;letter-spacing:0;`},()=>`${this.size.val}字`), ()=>{console.log(this.viewer.children);div(this.viewer.children)}))
        //van.add(document.body, div({style:()=>this.style()}, textarea({rows:5, cols:40, oninput:(e)=>this.manuscript.val=e.target.value}, ()=>this.manuscript.val), button({style:`word-break:break-all;padding:0;margin:0;line-height:1em;letter-spacing:0;`},()=>`${this.size.val}字`), ()=>this.viewer.el))
        //van.add(document.body, div({style:()=>this.style()}, textarea({rows:5, cols:40, oninput:(e)=>this.manuscript.val=e.target.value}, ()=>this.manuscript.val), button({style:`word-break:break-all;padding:0;margin:0;line-height:1em;letter-spacing:0;`},()=>`${this.size.val}字`), ()=>div(this.viewer.el)))
        //van.add(document.body, div({style:()=>this.style()}, textarea({rows:5, cols:40, oninput:(e)=>this.manuscript.val=e.target.value}, ()=>this.manuscript.val), button({style:`word-break:break-all;padding:0;margin:0;line-height:1em;letter-spacing:0;`},()=>`${this.size.val}字`), ()=>div({onwheel:(e)=>this.#onWheel(e), style:'writing-mode:vertical-rl;'}, this.htmls.val)))
