...
class JavelBodyWriter {
    constructor() {
        ...
        */
        this._anyoneBtn = new AnyOneButton(this.editor, this.exporter, this.colorScheme)
        this.menu = new MenuScreen([
            headBtn,
...
class AnyOneButton { // ButtonSelector 常にどれか一つのボタンだけ表示する
    constructor(editor, exporter, colorScheme) {
        ...
        this._expBtn = DivButton.make(()=>{this._exporter.export(this._editor.value);this.showAbout();this._expBtn.focus();this._toast.show('ファイル出力しました', {type:'success',position:'top-right',style:{backgroundColor:this._colorScheme.reverseBg, color:this._colorScheme.reverseFg, fontFamily:['Noto Sans JP', 'Source Han Sans JP', 'Noto Color Emoji', 'sans-serif']}});}, ()=>`出`)
