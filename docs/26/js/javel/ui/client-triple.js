...
class ClientTriple { // windowのclientにFitするdiv要素
    ...
    resize(width=0, height=0) {
        if (0===width) { width = window.clientSize.w }
        if (0===height) { height = window.clientSize.h }
...
class InDiv {
    constructor(children, col, isMin) {
        ...
        this._el = van.tags.div({
            ...
            ()=>van.tags.div({style:()=>`width:100%;height:100%;border:0;overflow:auto;font-size:1rem;line-height:1.7rem;letter-spacing:0.05rem;${this.#minStyle()}background-color:${col};${this._wm.css}${this._fs.css}`}, this._children.val))
