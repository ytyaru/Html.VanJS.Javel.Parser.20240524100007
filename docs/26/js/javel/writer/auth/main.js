...
https://web3.askmona.org/
    ...
    #makeCryptoTable() { return van.tags.table({style:'width:100%;height:100%;'},
        this._addCryptos.val.map(id=>van.tags.tr(
            van.tags.th({style:'width:3.5em;'},
                van.tags.a({tabindex:0, 'data-id':`delete-${id}`, style:'display:block;', onclick:()=>this.#deleteCryptoTr(id), onkeydown:(e)=>{if(' ,Enter'.split(',').includes(e.key)){this.#deleteCryptoTr(id)}}}, van.tags.ruby({style:'ruby-position:under;'},
    ...
    #deleteCryptoTr(id) {
        const value = document.querySelector(`#crypto-${id}-address`).value.trim()
        ...
        //if (confirm(`暗号資産 ${id} のアドレスを削除します。\n${value}\nよろしいですか？`)) {
        //}
    }
    #setUrls() {
