...
class JavelBodyWriter {
    constructor() {
        ...
        */
        this._anyoneBtn = new AnyOneButton(this.exporter, this.manuscript, this.colorScheme)
        this.menu = new MenuScreen([
            headBtn,
...
class AnyOneButton { // ButtonSelector 常にどれか一つのボタンだけ表示する
    constructor(exporter, manuscript, colorScheme) {
        ...
        //this._expBtn = DivButton.make(()=>{this.exporter.export(this.manuscript.val);this.isImmediatelyExport=true}, ()=>`出`)
        //this._expBtn = DivButton.make(()=>{this.isImmediatelyExport=true}, ()=>`出`)
        //this._expBtn = DivButton.make(()=>{this.showAbout()}, ()=>`出`)
        //this._expBtn = DivButton.make(()=>{this._exporter.export(this._manuscript.val);this.showAbout();}, ()=>`出`)
        //this._expBtn = DivButton.make(()=>{this.showAbout();this._exporter.export(this._manuscript.val);this.showAbout();}, ()=>`出`)
        //this._expBtn = DivButton.make(()=>{this.showAbout();this._expBtn.focus();}, ()=>`出`)
        ...
        //this._expBtn = DivButton.make(()=>{this._exporter.export(this._manuscript.val);this.showAbout();this._expBtn.focus();this._toast.show('ファイル出力しました', {type:'success',position:'top-right',style:{backgroundColor:Css.get('--background-color'), color:Css.get('--color')}});}, ()=>`出`)
        this._expBtn = DivButton.make(()=>{this._exporter.export(this._manuscript.val);this.showAbout();this._expBtn.focus();this._toast.show('ファイル出力しました', {type:'success',position:'top-right',style:{backgroundColor:this._colorScheme.reverseBg, color:this._colorScheme.reverseFg, fontFamily:['Noto Sans JP', 'Source Han Sans JP', 'Noto Color Emoji', 'sans-serif']}});}, ()=>`出`)
...
*/
        ...
        this._impBtn = DivButton.make(()=>{}, ()=>`入`)
        // 0===this.manuscript.length: 入
