...
//    constructor() { this._els=[];this._i=-1;this._activeElXpath=null;window.addEventListener('DOMContentLoaded', (event)=>this.setup()) }
    constructor() {
        ...
        this._i = -1
        this.events = {
            focus: new CustomEvent('changeFocus'),
            focused: new CustomEvent('changedFocus'),
        }
        this._activeElXpath=null
    ...
    #hasFocusIn() {
        ...
        for (let i=0; i<this._els.length; i++) {
            //if (this._els[i].hasOwnProperty('onfocusin') && 'function'===typeof this._els[i].onfocusin) {
            if ((this._els[i].onfocusin || this._els[i].hasOwnProperty('onfocusin')) && 'function'===typeof this._els[i].onfocusin) {
            //if (this._els[i].hasOwnProperty('onfocus') && 'function'===typeof this._els[i].onfocus) {
                console.log('持ってる')
            } else {
                console.log('focusinを持ってない', this._els[i])
                /*
                //console.log('focusを持ってない')
                this._els[i].addEventListener('focusin', (e)=>{
                //this._els[i].addEventListener('focus', (e)=>{
                    //window.dispatchEvent(new CustomEvent('changedFocus', {detail:{el:this._els[this._i], i:this._i}}))
                    window.dispatchEvent(new CustomEvent('changedFocus', {detail:{el:this._els[i], i:i}}))
                    //window.dispatchEvent(new CustomEvent('changedFocus', {detail:{el:this._els[i], i:this.index}}))
                })
                */
                this._els[i].onfocusin = (e)=>{
                    //window.dispatchEvent(new CustomEvent('changedFocus', {detail:{el:this._els[i], i:i}}))
                    //window.dispatchEvent(new CustomEvent('changedFocus', {detail:{el:this._els[this._i], i:this._i}}))
                    //const E = document.activeElement
                    //const I = this._els.indexOf(E)
                    //window.dispatchEvent(new CustomEvent('changedFocus', {detail:{el:E, i:I}}))
                    //window.dispatchEvent(new CustomEvent('changedFocus', {detail:{focuser:this}}))
                }
                this._els[i].addEventListener('focusin', (e)=>{
                    this._i = this.els.indexOf(document.activeElement)
                    //window.dispatchEvent(new CustomEvent('changedFocus', {detail:{el:this._els[this._i], i:this._i}}))
                    //window.dispatchEvent(new CustomEvent('changedFocus', {detail:{el:this._els[i], i:i}}))
                    //window.dispatchEvent(new CustomEvent('changedFocus', {detail:{el:this._els[i], i:this.index}}))
                    window.dispatchEvent(new CustomEvent('changedFocus', {detail:{focuser:this}}))
    ...
    set index(v) { // v:整数値(0〜this.els.length-1 または -1以下の負数(負数時は最後尾から数える。-1=最後尾, els.length*-1=先頭))
        ...
        //window.dispatchEvent(new CustomEvent('changeFocus', {detail:{el:this._els[this._i], i:this._i}}))
        this.elsi
