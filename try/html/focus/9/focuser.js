...
class Focuser {
    ...
    dispatch() { this.elsi; this.#dispatch(); }
    dispatched() { this.elsi; this.#dispatched(); }
    #dispatch() { this.#_dispatch(this._eventNames.focus) }
    #dispatched() { this.#_dispatch(this._eventNames.focused) }
    #_dispatch(N) { window.dispatchEvent(new CustomEvent(N, {detail:{el:this._els[this._i], i:this._i, focuser:this}})) }
    listen(fn) { window.addEventListener(this._eventNames.focus, fn) }
    listened(fn) { window.addEventListener(this._eventNames.focused, fn) }
    unlisten(fn) { window.removeEventListener(this._eventNames.focus, fn) }
    unlistened(fn) { window.removeEventListener(this._eventNames.focused, fn) }
}
