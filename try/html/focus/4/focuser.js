...
class Focuser {
    ...
    set index(v) {
        ...
        if (v < els.length) {
//            if (0<=v) { els[v].focus() }
//            const i = (0<=v) ? v : els.length + v
            els[(0<=v) ? v : els.length + v].focus()
        }
    }
    ...
    #setFocusToFirstNode() { const els = this.els; if (0 < els.length) { els[0].focus() } }
    focus(e) {
        ...
        if (!document.contains(document.activeElement)) { nodes[0].focus() }
            ...
            if (e.shiftKey && focusedItemIndex === 0) {
                ...
                e.preventDefault() // indexがずれる！たぶんここでactiveEventのセットが実行されなくなる
            }
